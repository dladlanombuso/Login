...
public class Login {
    ...
    public boolean checkCellPhoneNumber(String Phone) {
       ...
       // 4. registerUser() - Processes registration and returns a status  message
       public String registerUser(String username, String password, String phone) {
           if (!checkUserName(username)) {
               return "Username is not correctly formatted; please ensure that your username contains an underscore and is no more than five characters in length.";
           } else if (!checkPasswordComplexity(password)) {
               return "Password is not correctly formatted; please ensure that the password cantains at least eight characters, a capital letter, a number, and a special character.";
           } else if (!checkCellPhoneNumber(phone)) {
               return "Cell phone number incorrectly formatted or does  not cotian international code.";
           } else {
       ...
       // 5. loginUser() - Verifies if  credentials match the registered ones
       public boolean loginUser(String username, String password) {
           return username.equals(this.registeredUsername) & password.equals(this.registeredPassword);
       }
       
       // 6. returnLoginStatus() - Returns the final message after a login attempt
       ...
       public static void main(String[] args) {
           ...
           System.out.println("Enter CellPhoneNumber:+27767373378 ");
